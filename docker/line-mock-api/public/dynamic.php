...
// ランダムテキスト生成
function generateRandomTitle(int $seed, array $tags = []): string
{
    mt_srand($seed);
    ...
    if (!empty($tags)) {
        ...
        for ($i = 0; $i < $tagCount && !empty($availableTags); $i++) {
            $index = array_rand($availableTags);
            $selectedTags[] = $availableTags[$index];
            unset($availableTags[$index]);
        }

        if (!empty($selectedTags)) {
            $title .= ' #' . implode(' #', $selectedTags);
        }
    }
...
function generateRandomDescription(int $seed, array $tags = []): string
{
    mt_srand($seed);
...
// データ読み込み・初期化
function loadOrInitializeData(string $dataFile, int $count, string $language, array $tags = []): array
{
    ...
    foreach ($categoryCounts as $categoryId => $categoryCount) {
        for ($i = 0; $i < $categoryCount; $i++) {
            ...
            // 各カテゴリの最初の4件に特別な値を設定（emblem=1,2 と joinMethodType=1,2）
            $emblem = ($i === 0) ? 1 : (($i === 1) ? 2 : rand(0, 1));
            $joinMethodType = ($i === 2) ? 1 : (($i === 3) ? 2 : rand(0, 1));

            $rooms[] = [
                'emid' => $emid,
                'name' => generateRandomTitle($emidSeed, $tags),
                'desc' => generateRandomDescription($emidSeed + 100, $tags),
...
try {
    ...
    if (preg_match('#^/api/category/(\d+)\?sort=(RANKING|RISING)&limit=(\d+)(?:&ct=(.*))?$#', $requestUri, $matches)) {
        ...
        if ($sort === 'RANKING') {
            ...
            $allRooms = loadOrInitializeData($dataFile, $rankingCount, $language, $tags);
        } else {
            $dataFile = sprintf($risingDataFile, $language);
            $allRooms = loadOrInitializeData($dataFile, $risingCount, $language, $tags);
        }
    ...
    if (preg_match('#^/api/square/([a-zA-Z0-9_-]+)\?limit=1$#', $requestUri, $matches)) {
        ...
        $allRooms = array_merge(
            loadOrInitializeData($rankingFile, $rankingCount, $language, $tags),
            loadOrInitializeData($risingFile, $risingCount, $language, $tags)
        );
    ...
    if (preg_match('#^/(jp|tw|th)?/?ti/g2/([a-zA-Z0-9_-]+)$#', $requestUri, $matches)) {
        ...
        $allRooms = array_merge(
            loadOrInitializeData($rankingFile, $rankingCount, $pageLang, $tags),
            loadOrInitializeData($risingFile, $risingCount, $pageLang, $tags)
        );
