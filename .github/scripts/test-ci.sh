...
run_cron_job() {
    ...
    if [ -z "$lang_arg" ]; then
        # 日本語（引数なし）
        docker exec --user www-data "$APP_CONTAINER" bash -c \
...
main() {
    ...
    docker exec --user www-data -i "$APP_CONTAINER" /usr/local/bin/php batch/exec/ocreview_api_data_import_background.php 2>&1 | tee "$LOG_DIR/api_data_import.log"
