    ...
    <div class="body">
        ...
        <main style="overflow: hidden;">
            <article class="terms">
                ...
                <?php
                    $deletedTotal = $deletedRoomsMonthly + $delistedMonthly['rooms'];
                ...
                <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-4 sm:p-5 mb-10">
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm sm:text-base font-bold text-gray-800"><?php echo number_format($newRoomsMonthly) ?>部屋</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-xs sm:text-sm text-gray-600">参加者数の純増数</span>
                            <span class="text-sm sm:text-base font-bold <?php echo trendColorClass($netMemberChange) ?>"><?php echo ($netMemberChange >= 0 ? '+' : '') . number_format($netMemberChange) ?></span>
                        </div>
                        <div class="pl-4 space-y-1">
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-gray-400">既存ルームの増加</span>
                                <span class="text-xs font-semibold text-emerald-600">+<?php echo number_format($b['increased']) ?></span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-gray-400">新規ルーム分</span>
                                <span class="text-xs font-semibold text-emerald-600">+<?php echo number_format($b['gained']) ?></span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-gray-400">既存ルームの減少</span>
                                <span class="text-xs font-semibold text-rose-600"><?php echo number_format($b['decreased']) ?></span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-gray-400">消滅ルーム分</span>
                                <span class="text-xs font-semibold text-rose-600"><?php echo number_format($b['lost']) ?></span>
                            </div>
                        </div>
                        <?php $deletedMembers = -$b['lost'] - $delistedMonthly['members'] ?>
                        <div class="flex items-center justify-between border-t border-gray-100 pt-3">
                            <span class="text-xs sm:text-sm text-gray-600">削除されたルーム数</span>
                            <span class="text-sm sm:text-base font-bold text-rose-600"><?php echo number_format($deletedTotal) ?>部屋 <span class="text-xs font-normal text-rose-400">(-<?php echo number_format(-$b['lost']) ?>人)</span></span>
                        </div>
                        <div class="pl-4 space-y-1">
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-gray-400">閉鎖ルーム数</span>
                                <span class="text-xs font-semibold text-gray-500"><?php echo number_format($deletedRoomsMonthly) ?>部屋 <span class="font-normal text-gray-400">(-<?php echo number_format($deletedMembers) ?>人)</span></span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-gray-400">LINE公式サイト掲載終了</span>
                                <span class="text-xs font-semibold text-gray-500"><?php echo number_format($delistedMonthly['rooms']) ?>部屋 <span class="font-normal text-gray-400">(-<?php echo number_format($delistedMonthly['members']) ?>人)</span></span>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                <?php foreach ($categoryStats as $i => $cat): ?>
                                    <tr class="<?php echo $i % 2 === 0 ? 'bg-white' : 'bg-slate-50' ?> hover:bg-blue-50 transition-colors">
                                        ...
                                        <td class="py-2.5 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-semibold <?php echo trendColorClass($cat['monthly_trend']) ?> text-right tabular-nums border-b border-gray-100"><?php echo ($cat['monthly_trend'] >= 0 ? '+' : '') . number_format($cat['monthly_trend']) ?></td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
                        </table>
